...
export class ImperativeBetterStrategy {
  ...
  createBreakElem(lineElement, order) {
    ...
    breakEl.className = "blyrics--break";
  ...
  mount(domContainer, lyrics, extraData: any = {}) {
    ...
    const { romanizedLyrics, translatedLyrics } = extraData;
    ...
    lyrics.forEach((line, lineIndex) => {
      ...
      if (romanized) {
        this.createBreakElem(lineDiv, 4);
      ...
      if (translated) {
        this.createBreakElem(lineDiv, 6);
  ...
  updateSecondaryLyrics(extraData: any = {}) {
    ...
    this.lines.forEach((lineData, lineIndex) => {
      ...
      // 1. Update Romanized
      const romanized = romanizedLyrics?.[lineIndex]?.romanized;
      const romanizedEl = lineDiv.querySelector(".blyrics--romanized") as HTMLElement;
      const break4 = lineDiv.querySelector('.blyrics--break[style*="order: 4"], .blyrics--break[style*="order:4"]');
      ...
      // 2. Update Translated
      const translated = translatedLyrics?.[lineIndex]?.translated;
      const translatedEl = lineDiv.querySelector(".blyrics--translated") as HTMLElement;
      const break6 = lineDiv.querySelector('.blyrics--break[style*="order: 6"], .blyrics--break[style*="order:6"]');
      ...
      // If line is currently active/selected/animating, force re-animation so newly attached romanized spans start swiping immediately
      if (lineData.isSelected || lineData.isAnimating) {
